...
typedef struct {
    ...
    XFm_synth_uio_map maps[ MAX_UIO_MAPS ];
} XFm_synth_uio_info;

/***************** Variable Definitions **************************************/
static XFm_synth_uio_info uio_info;
...
static int uio_info_read_name(XFm_synth_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XFm_synth_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XFm_synth_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XFm_synth_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XFm_synth_Initialize(XFm_synth *InstancePtr, const char* InstanceName) {
	XFm_synth_uio_info *InfoPtr = &uio_info;
